...
from load_data import pad, load
from feature_select import select


def experiment(feature_name='audio', update='adam2', lamb=0., drop=0., activation=None, sq_loss=False,
               model='lstml', share=False, category=True, maxlen=1000, sample_rate=5,
    ...
    for doc in tests:
        if doc not in session_Xs:
            continue
        Xs_test_list += session_Xs[doc]
        y_test_list += session_y[doc]

    Xs_train_list, y_train_list = [], []
    for doc in trains:
        if doc not in session_Xs:
            continue
        Xs_train_list += session_Xs[doc]
        y_train_list += session_y[doc]

    if feat_sel:
        feature_indices = select(Xs_train_list + Xs_test_list, y_train_list + y_test_list, ratio=ratio, use_mean=use_mean)
